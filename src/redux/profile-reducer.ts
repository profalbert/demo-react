...
export const profileActions = {
  ...
  deletePost: (postId: number) =>
    ({
      // используется для теста в PR
      type: 'profile/DELETE_POST',
      postId,
    } as const),
}

export const setUserProfileThunk = (userId: number): ThunkType => async (
  dispatch,
) => {
...
export const getStatusThunk = (userId: number): ThunkType => async (
  dispatch,
) => {
  try {
...
export const updateStatusThunk = (status: string): ThunkType => async (
  dispatch,
) => {
  try {
...
export const saveProfileThunk = (profile: ProfileType): ThunkType => async (
  dispatch,
  getState,
) => {
  ...
  if (response.resultCode === ResultStandartCodes.Success) {
    ...
    // const action = stopSubmit("edit-profile", {"contacts": {"facebook": response.data.messages[0]}}); // выводит ошибку у конкретного поля
    const action = stopSubmit('edit-profile', {
      _error: 'Error! Please check the entered data!',
    })
    dispatch(action)
    return Promise.reject(response.messages[0])
  }

...
export const authActions = {
  setAuthUserData: (
    ...
    ({
      ...
      payload: { userId, email, login, isAuth, captchaUrl, photos },
    } as const),
  ...
  setPhotosMeSuccess: (photos: PhotosType) =>
    ({
      type: 'auth/SET_PHOTOS_ME_SUCCESS',
      payload: { photos },
    } as const),
}
...
export const setPhotosMeThunk = (myId: number): ThunkType => async (
  dispatch,
) => {
  const response = await authAPI.photoMe(myId)

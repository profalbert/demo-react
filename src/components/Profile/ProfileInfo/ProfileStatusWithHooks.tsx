...
const ProfileStatusWithHooks: React.FC<PropsType> = ({
  ...
  useEffect(() => {
    // срабатывает в самом конце компоненты
  ...
  const ownerExpression = isOwner || idComparison
  ...
  const deactivateEditMode = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.currentTarget.value.length <= 300) {
      setEditMode(false)
      new Promise(() => updateStatus(status)).catch((error: string) => {
  ...
  const onStatusChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      e.currentTarget.value.length === 301 ||
      e.currentTarget.value.length > 301
    ) {
      ...
      {!editMode ? (
        <div className={s.statusFromProps}>
          <span onDoubleClick={activateMode}>
            {statusFromProps ? `${statusFromProps}` : `no information`}
          </span>
        </div>
      ) : (
        <div className={s.statusFromState}>
          ...
          {ownerExpression && (
            <input
              className={s.statusFromStateInput}

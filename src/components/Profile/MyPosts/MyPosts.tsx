...
const MyPosts = React.memo<PropsType>((props) => {
  const postsElements = props.posts.map((p: PostType) => (
    <Post key={p.id} message={p.message} likesCount={p.likesCount} />
  ))

  const onAddPost = (values: AddPostNewFormValuesType) => {
  ...
  return (
    <div className={s.postsBlock}>
      ...
      <div className={s.posts}>{postsElements.reverse()}</div>
    </div>
  )
})
...
type AddPostNewFormValuesKeysType = Extract<
  keyof AddPostNewFormValuesType,
  string
>
const maxLengthCreator10 = maxLengthCreator(10)

const AddPostNewForm: React.FC<InjectedFormProps<AddPostNewFormValuesType>> = ({
  handleSubmit,
}) => {
  return (
    <form className={s.formMyPosts} onSubmit={handleSubmit}>
      <div>
        {createField<AddPostNewFormValuesKeysType>(
          'Post message',
          'newPostText',
          [required, maxLengthCreator10, withoutSpace],
          Textarea,
          'text',
          {
